...
public class LoginDialog extends IdeaCodeActivity {
	...
	private RelativeLayout loginLoading = null;
	...
	private void login(final String account, final String pwd,
		...
		HashMap<String, TbUser> params = new HashMap<String, TbUser>();
	...
	public void refresh(Object... param) {
		...
		case TaskType.TS_EXAM_LOGIN:
			if (param[2] != null
					&& (Integer) param[2] == AppException.TYPE_SOAP) {
				...
				if (param[1] != null
						&& Integer.parseInt(hashMap.get("code").toString()) == 1) {
					TbUser tbUser = new TbUser();
					tbUser.setUserId(Long.parseLong(hashMap.get("userId")
							.toString()));
					tbUser.setUserName(hashMap.get("userName").toString());
					tbUser.setUserPassword(mPwd.getText().toString());
					tbUser.setLocation("");
					tbUser.setRememberMe(Boolean.parseBoolean(hashMap.get(
					...
					LoginDialog.this.finish();
						...
						&& Integer.parseInt(hashMap.get("code").toString()) == 0) {
					appContext.cleanLoginInfo();
