...
import android.demo.news.db.DataHelper;
...
public class TabMoodActivity extends IdeaCodeActivity {
	...
	private void initMoodListView() {
		lvMoodAdapter = new ListViewMoodAdapter(this, this, lvMoodData,
				R.layout.mood_listitem);
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (view instanceof TextView) {
					...
					TextView tv = (TextView) view
							.findViewById(R.id.mood_listitem_content);
					mood = (Mood) tv.getTag();
				}
				if (mood == null)
					return;
	...
	public void doPraise(View view) {
		...
		if (appContext.isLogin()) {
			...
			int code = datahelp.addUserPraiseBelittle(appContext.getLoginUid(),
					mood.getMoodId(), UserOptionType.ADD_PRAISE);
			if (code == CommonSetting.Success) {
				int praiseCount = mood.getMoodPraiseCount() + 1;
				TextView praiseCountTv = (TextView) view
						.findViewById(R.id.mood_praise_count);
	...
	public void doBelittle(View view) {
		...
		if (appContext.isLogin()) {
			Mood mood = (Mood) view.getTag();
			int code = datahelp.addUserPraiseBelittle(appContext.getLoginUid(),
					mood.getMoodId(), UserOptionType.ADD_BELITTLE);
			if (code == CommonSetting.Success) {
				int belittleCount = mood.getMoodBelittleCount() + 1;
				TextView belittleeCountTv = (TextView) view
						.findViewById(R.id.mood_belittle_count);
	...
	public void refresh(Object... param) {
		...
		case TaskType.TS_EXAM_SEARCH_MOOD:
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (moodList == null || moodList.size() == 0) {
					...
					lvMoodData = moodList;
					lvMoodAdapter = new ListViewMoodAdapter(this, this,
							lvMoodData, R.layout.mood_listitem);
					lvMood.setAdapter(lvMoodAdapter);
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (moodList == null || moodList.size() == 0) {
					...
					HeaderViewListAdapter listAdapter = (HeaderViewListAdapter) lvMood
							.getAdapter();
					ListViewMoodAdapter resultadapter = (ListViewMoodAdapter) listAdapter
							.getWrappedAdapter();
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (code == CommonSetting.Success) {
					UIHelper.ToastMessage(this,
							R.string.msg_mood_addpraise_success);
				} else {
					UIHelper.ToastMessage(this,
							R.string.msg_mood_addpraise_error);
				}
			}
			break;
		case TaskType.TS_EXAM_BELITTLE_MOOD:
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (code == CommonSetting.Success) {
					UIHelper.ToastMessage(this,
							R.string.msg_mood_addbelittle_success);
				} else {
					UIHelper.ToastMessage(this,
							R.string.msg_mood_addbelittle_error);
				}
			}
			break;
		}
