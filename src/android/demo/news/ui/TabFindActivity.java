...
import android.demo.news.adapter.ListViewPopFavouriteNewsAdapter;
import android.demo.news.adapter.ListViewPopMoodAdapter;
...
public class TabFindActivity extends IdeaCodeActivity {
	...
	private void initPopMoodViews() {
		lvMoodAdapter = new ListViewPopMoodAdapter(this, lvMoodData,
				R.layout.popmood_listitem);
	...
	private void initPopFavouriteViews() {
		lvFavouriteAdapter = new ListViewPopFavouriteNewsAdapter(this,
				lvNewsData, R.layout.popnews_listitem);
		...
		lvFavourite
				.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						// 跳转到新闻详情
						Intent intent = new Intent(TabFindActivity.this,
	...
	private void loadPopFavouriteList(int action) {
		...
		if (action == UIHelper.LISTVIEW_ACTION_REFRESH) {
			...
			params.put("paging", p);
			params.put("isRefresh", isRefresh);
			Task ts = new Task(TaskType.TS_EXAM_SEARCH_POP_FAVOURITE_MORE,
					params);
			MainService.newTask(ts);
		} else {
			Paging p = new Paging(favouriteNowpage, AppContext.PAGE_SIZE);
		...
		case TaskType.TS_EXAM_SEARCH_POP_MOOD:
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (moodList == null || moodList.size() == 0) {
					lvMoodData = moodList;
					lvMoodAdapter = new ListViewPopMoodAdapter(this,
							lvMoodData, R.layout.popmood_listitem);
					lvMood.setAdapter(lvMoodAdapter);
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (moodList == null || moodList.size() == 0) {
					...
					HeaderViewListAdapter listAdapter = (HeaderViewListAdapter) lvMood
							.getAdapter();
					ListViewPopMoodAdapter resultadapter = (ListViewPopMoodAdapter) listAdapter
							.getWrappedAdapter();
		...
		case TaskType.TS_EXAM_SEARCH_POP_FAVOURITE:
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (newsList == null || newsList.size() == 0) {
					lvNewsData = newsList;
					lvFavouriteAdapter = new ListViewPopFavouriteNewsAdapter(
							this, lvNewsData, R.layout.popnews_listitem);
					lvFavourite.setAdapter(lvFavouriteAdapter);
