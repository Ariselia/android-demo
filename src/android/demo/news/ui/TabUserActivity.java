...
import android.demo.news.adapter.ListViewNewsAdapter;
import android.demo.news.adapter.ListViewUserMoodAdapter;
import android.demo.news.app.AppContext;
import android.demo.news.bean.Mood;
import android.demo.news.bean.Paging;
import android.demo.news.bean.TbUser;
import android.demo.news.common.util.CommonSetting;
import android.demo.news.common.util.StringUtils;
import android.demo.news.common.util.UIHelper;
import android.demo.news.listener.ChoiceOnClickListener;
import android.demo.news.listener.DialogButtonClickListener;
import android.demo.news.logic.IdeaCodeActivity;
import android.demo.news.logic.MainService;
import android.demo.news.logic.Task;
import android.demo.news.logic.TaskType;
import android.demo.news.widget.CustomDialog;
import android.demo.news.widget.NewDataToast;
import android.demo.news.widget.PullToRefreshListView;
import android.demo.news.widget.ScrollLayout;
...
public class TabUserActivity extends IdeaCodeActivity {
	...
	// 用户信息相关对象声明
	private static TextView username_tv, useremail_tv, usersex_tv,
			userlocation_tv, usersummary_tv, birthday_tv, qq_tv;
	...
	private void initUserMoodViews() {
		lvMoodAdapter = new ListViewUserMoodAdapter(this, lvMoodData,
				R.layout.usermood_listitem);
		lvMood_footer = getLayoutInflater().inflate(R.layout.listview_footer,
				null);
		lvMood_foot_more = (TextView) lvMood_footer
				.findViewById(R.id.listview_foot_more);
		lvMood_foot_more.setText(R.string.load_more);
		lvMood_foot_progress = (ProgressBar) lvMood_footer
		...
		lvMood.setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				try {
					if (view.getPositionForView(lvMood_footer) == view
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				lvMood.onScroll(view, firstVisibleItem, visibleItemCount,
						totalItemCount);
			}
		});
	...
	private void initFavouriteViews() {
		lvFavouriteAdapter = new ListViewNewsAdapter(this, lvNewsData,
				R.layout.news_listitem);
		lvFavourite_footer = getLayoutInflater().inflate(
				R.layout.listview_footer, null);
		lvFavourite_foot_more = (TextView) lvFavourite_footer
				.findViewById(R.id.listview_foot_more);
		lvFavourite_foot_more.setText(R.string.load_more);
		lvFavourite_foot_progress = (ProgressBar) lvFavourite_footer
				.findViewById(R.id.listview_foot_progress);
		...
		lvFavourite
				.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						// 跳转到新闻详情
						Intent intent = new Intent(TabUserActivity.this,
								NewsDetailActivity.class);
						intent.putExtra("newsType", Integer.parseInt(newsTitle
								.get("newsType").toString()));
						intent.putExtra("newsDetail_url", newsTitle.get("url")
								.toString());
		...
		lvFavourite.setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				boolean scrollEnd = false;
				try {
					if (view.getPositionForView(lvFavourite_footer) == view
							.getLastVisiblePosition()) {
						scrollEnd = true;
					}
				} catch (Exception e) {
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
					int visibleItemCount, int totalItemCount) {
				lvFavourite.onScroll(view, firstVisibleItem, visibleItemCount,
						totalItemCount);
			}
		});
		lvFavourite
	...
	private final OnClickListener onClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.submit_bt:
				...
				tbUser.setUserBirthday(StringUtils.toDate2(birthday_tv
						.getText().toString()));
	...
	private void loadUserFavouriteList(int action) {
		...
		if (action == UIHelper.LISTVIEW_ACTION_REFRESH) {
			...
			params.put("paging", p);
			params.put("isRefresh", isRefresh);
			Task ts = new Task(TaskType.TS_EXAM_SEARCH_USER_FAVOURITE_MORE,
					params);
			MainService.newTask(ts);
		} else {
			Paging p = new Paging(favouriteNowpage, AppContext.PAGE_SIZE);
	...
	protected Dialog onCreateDialog(int id) {
		...
		case SEX_PICK_DIALOG:
			...
			DialogInterface.OnClickListener btnListener = new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialogInterface, int which) {
					...
					String sexStr = getResources().getStringArray(
							R.array.sexArray)[choiceWhich];
		...
		case LOCATION_PICK_DIALOG:
			CustomDialog.Builder customBuilder = new CustomDialog.Builder(
					TabUserActivity.this, provinceid, cityid);
	...
	public static void checkUpdate() {
		if (!tbUser.getUserName().equals(username_tv.getText().toString())
				|| !tbUser.getUserEmail().equals(
						useremail_tv.getText().toString())
				|| !tbUser.getUserSummary().equals(
						usersummary_tv.getText().toString())
				|| !tbUser.getUserQQ().equals(qq_tv.getText().toString())
				|| !tbUser.getUserSex().equals(usersex_tv.getText().toString())
				|| !StringUtils.date2String(tbUser.getUserBirthday()).equals(
						birthday_tv.getText().toString())
				|| provinceid != CommonSetting.ids[0]
				|| cityid != CommonSetting.ids[1]) {
			if (provinceid != CommonSetting.ids[0]
					|| cityid != CommonSetting.ids[1]) {
				userlocation_tv
						.setText(CommonSetting.provinces[CommonSetting.ids[0]]
								.getName()
								+ "  "
								+ CommonSetting.cities[CommonSetting.ids[0]][CommonSetting.ids[1]]
										.getName());
			}
	...
	public void refresh(Object... param) {
		...
		case TaskType.TS_EXAM_SEARCH_USER_MOOD:
			lvMood.onRefreshComplete(getString(R.string.pull_to_refresh_update)
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (moodList == null || moodList.size() == 0) {
					lvMoodData = moodList;
					lvMoodAdapter = new ListViewUserMoodAdapter(this,
							lvMoodData, R.layout.usermood_listitem);
					lvMood.setAdapter(lvMoodAdapter);
		...
		case TaskType.TS_EXAM_SEARCH_USER_MOOD_MORE:
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (moodList == null || moodList.size() == 0) {
					...
					lvMoodData = moodList;
					HeaderViewListAdapter listAdapter = (HeaderViewListAdapter) lvMood
							.getAdapter();
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (tbUser.getUserName() == null) {
					UIHelper.ToastMessage(this,
							R.string.msg_getuserinfo_failure);
				} else {
					...
					if (provinceid != -1 && cityid != -1) {
						userlocation_tv.setText(CommonSetting.provinces[tbUser
								.getUserProvinceId()].getName()
								+ "  "
								+ CommonSetting.cities[tbUser
										.getUserProvinceId()][tbUser
										.getUserCityId()].getName());
					} else {
						userlocation_tv.setText("");
					}
					usersummary_tv.setText(tbUser.getUserSummary());
					String birthday = StringUtils.date2String(tbUser
							.getUserBirthday());
		...
		case TaskType.TS_EXAM_SEARCH_USER_FAVOURITE:
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (newsList == null || newsList.size() == 0) {
					lvNewsData = newsList;
					lvFavouriteAdapter = new ListViewNewsAdapter(this,
							lvNewsData, R.layout.news_listitem);
					lvFavourite.setAdapter(lvFavouriteAdapter);
		...
		case TaskType.TS_EXAM_SEARCH_USER_FAVOURITE_MORE:
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (newsList == null || newsList.size() == 0) {
					...
					lvNewsData = newsList;
					HeaderViewListAdapter listAdapter = (HeaderViewListAdapter) lvFavourite
							.getAdapter();
					ListViewNewsAdapter resultadapter = (ListViewNewsAdapter) listAdapter
							.getWrappedAdapter();
