...
public class FeedBack extends IdeaCodeActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mPoi_on_icon = getResources().getDrawable(
				R.drawable.addmood_poiactive_icon);
	...
	private void initViews() {
		...
		moodAddPoint = FeedBack.this.getResources().getString(
				R.string.mood_add_point);
		moodPointing = FeedBack.this.getResources().getString(
				R.string.mood_pointting);
		...
		mSend.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (mContent.getText().toString().trim().length() == 0) {
					UIHelper.ToastMessage(FeedBack.this,
							R.string.mood_send_empty);
				} else {
					if (mContent.getText().length() > 150) {
						UIHelper.ToastMessage(FeedBack.this,
								R.string.feedback_send_toolong_error);
					} else {
						TbFeedBack tbFeedback = new TbFeedBack();
						tbFeedback.setUserId(application.isLogin() ? application
								.getLoginUid() : -1);
						tbFeedback.setFeedbackTitle(FeedBack.this
								.getResources().getString(
										R.string.feedback_def_title));
						tbFeedback.setFeedbackContent(mContent.getText()
								.toString());
						if (mPlace.getText().toString().equals(moodAddPoint)
							...
							tbFeedback.setFeedbackLoc(mPlace.getText()
									.toString());
						}
						...
						Task ts = new Task(TaskType.TS_EXAM_SENDFEEDBACK,
								params);
			...
			public void onClick(View v) {
				...
				if (mPlace.getText().toString().equals(moodAddPoint)) {
					...
					if (mClient.isStarted()) {
						...
						mLBSAddress = null;
						mPlace.setCompoundDrawables(mPoi_off_icon, null, null,
								null);
			...
			if (param[2] != null && (Integer) param[2] != 0) {
				...
				if (code == CommonSetting.Success) {
					...
					UIHelper.ToastMessage(this,
							R.string.msg_sendfeedback_success);
				} else {
