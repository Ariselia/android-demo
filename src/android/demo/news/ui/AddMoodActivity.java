...
import android.demo.news.adapter.GridViewFaceAdapter;
...
public class AddMoodActivity extends IdeaCodeActivity {
	...
	private void initViews() {
		...
		moodAddPoint = AddMoodActivity.this.getResources().getString(
				R.string.mood_add_point);
		moodPointing = AddMoodActivity.this.getResources().getString(
	...
	private void setListener() {
		...
		mSend.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (mContent.getText().toString().trim().length() == 0) {
					UIHelper.ToastMessage(AddMoodActivity.this,
							R.string.mood_send_empty);
				} else {
					if (mContent.getText().length() > 300) {
						UIHelper.ToastMessage(AddMoodActivity.this,
								R.string.mood_send_toolong_error);
					} else {
						Mood mood = new Mood();
						mood.setUserId(application.getLoginUid());
						mood.setUserName(application.getLoginInfo()
								.getUserName());
