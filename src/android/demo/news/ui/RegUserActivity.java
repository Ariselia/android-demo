...
import android.demo.news.common.util.MD5;
...
public class RegUserActivity extends IdeaCodeActivity {
	...
	private EditText reg_user_edit, reg_email_edit, reg_birthday_edit,
			reg_password_edit, reg_password2_edit;
	...
	private void initViews() {
		...
		reg_birthday_edit = (EditText) this
				.findViewById(R.id.reg_birthday_edit);
		reg_birthday_edit.setOnTouchListener(touchListener);
		reg_password_edit = (EditText) this
				.findViewById(R.id.reg_password_edit);
		reg_password2_edit = (EditText) this
				.findViewById(R.id.reg_password2_edit);
	...
	private final Button.OnClickListener buttonClickListener = new Button.OnClickListener() {
		...
		public void onClick(View view) {
			((InputMethodManager) getSystemService(INPUT_METHOD_SERVICE))
					.hideSoftInputFromWindow(RegUserActivity.this
			...
			case R.id.main_head_save_button:
				if ("".equals(reg_user_edit.getText().toString())
						|| "".equals(reg_email_edit.getText().toString())
						|| "".equals(reg_birthday_edit.getText().toString())
						|| "".equals(reg_password_edit.getText().toString())
						|| "".equals(reg_password2_edit.getText().toString())) {
					UIHelper.ToastMessage(RegUserActivity.this,
							R.string.regEmptyError);
					return;
				}
				if (StringUtils.checkUsernameInput(reg_user_edit.getText()
						.toString())) {
					if (StringUtils.checkEmailInput(reg_email_edit.getText()
							.toString())) {
						if (StringUtils.check2Password(reg_password_edit
								.getText().toString(), reg_password2_edit
								.getText().toString())) {
							TbUser tbUser = new TbUser();
							tbUser.setUserId(Long.parseLong(StringUtils
									.date2UserId()));
							tbUser.setUserBirthday(StringUtils
									.toDate2(reg_birthday_edit.getText()
											.toString()));
							tbUser.setUserName(reg_user_edit.getText()
									.toString());
							tbUser.setUserEmail(reg_email_edit.getText()
									.toString());
							tbUser.setUserSex(sex);
							tbUser.setUserPassword(MD5
									.MD5Encode(reg_password2_edit.getText()
											.toString()));// 通过MD5加密
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DATE_PICK_DIALOG:
			if ("".equals(reg_birthday_edit.getText().toString())) {
				...
				String[] dates = reg_birthday_edit.getText().toString()
						.split("-");
				dialog = new DatePickerDialog(this,
						new DatePickerDialog.OnDateSetListener() {
							@Override
