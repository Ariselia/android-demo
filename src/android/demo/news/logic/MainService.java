package android.demo.news.logic;
...
import android.demo.news.app.AppManager;
import android.demo.news.bean.Mood;
import android.demo.news.bean.NewsDetail;
import android.demo.news.bean.Paging;
import android.demo.news.bean.TbFeedBack;
import android.demo.news.bean.TbUser;
import android.demo.news.common.util.CommonSetting;
import android.demo.news.module.util.AppStartUtil;
import android.demo.news.module.util.FeedBackUtil;
import android.demo.news.module.util.FindUtil;
...
import android.demo.news.module.util.RegUtil;
...
public class MainService extends Service implements Runnable {
	...
	private void doTask(Task ts) {
		...
		case TaskType.TS_EXAM_GETINITIALIZEDATA: // 闪屏界面时，获取assset下面的初始化数据
			IdeaCodeActivity appStart = (IdeaCodeActivity) ts.getTaskParam()
					.get("context");
			try {
		...
		case TaskType.TS_EXAM_SEARCH_NEWS: // 查询新闻
			try {
				...
				int currentpage = (Integer) ts.getTaskParam()
						.get("currentpage");
				boolean isRefresh = (Boolean) ts.getTaskParam()
						.get("isRefresh");
				ArrayList<Map<String, Object>> newsList = NewsUtil
						.getNewsForList(this, newsType, currentpage, isRefresh);
		...
		case TaskType.TS_EXAM_SEARCH_NEWS_DETAIL_LOAD:// 查询新闻详细数据
			try {
				int newsType = (Integer) ts.getTaskParam().get("newsType");
				String newsDetail_url = (String) ts.getTaskParam().get(
						"newsDetail_url");
				boolean isRefresh = (Boolean) ts.getTaskParam()
						.get("isRefresh");
				NewsDetail newsDetail = NewsUtil.getNewsDetailByUrl(this,
						newsType, newsDetail_url, isRefresh);
		...
		case TaskType.TS_EXAM_NEWS_FAVOURITE: // 收藏新闻
			try {
				...
				NewsDetail newsDetail = (NewsDetail) ts.getTaskParam().get(
						"newsDetail");
		...
		case TaskType.TS_EXAM_SEARCH_MOOD: // 查询心情
			try {
				...
				boolean isRefresh = (Boolean) ts.getTaskParam()
						.get("isRefresh");
				ArrayList<Mood> moodList = MoodUtil.getMoodForList(this, p,
						isRefresh);
		...
		case TaskType.TS_EXAM_SEARCH_USER_MOOD: // 查询用户心情
			try {
				...
				boolean isRefresh = (Boolean) ts.getTaskParam()
						.get("isRefresh");
				ArrayList<Mood> userMoodList = MoodUtil.getUserMoodForList(
						this, userId, p, isRefresh);
		...
		case TaskType.TS_EXAM_SEARCH_USER_FAVOURITE: // 查询用户收藏的新闻
			try {
				...
				boolean isRefresh = (Boolean) ts.getTaskParam()
						.get("isRefresh");
				ArrayList<Map<String, Object>> newsList = MemberUtil
						.getUserFavouriteForList(this, userId, p, isRefresh);
				message.obj = newsList;
			} catch (AppException e) {
				message.arg1 = AppException.TYPE_SOAP;
			}
		...
		case TaskType.TS_EXAM_SENDFEEDBACK: // 发送用户反馈信息
			try {
				TbFeedBack tbFeedBack = (TbFeedBack) ts.getTaskParam().get(
						"feedback");
		...
		case TaskType.TS_EXAM_SEARCH_POP_MOOD: // 查询热门心情
			try {
				...
				boolean isRefresh = (Boolean) ts.getTaskParam()
						.get("isRefresh");
				ArrayList<Mood> moodList = FindUtil.getPopMoodForList(this, p,
						isRefresh);
				message.obj = moodList;
			} catch (AppException e) {
				message.arg1 = CommonSetting.SoapException;
		...
		case TaskType.TS_EXAM_SEARCH_POP_FAVOURITE: // 查询热门收藏
			try {
				...
				boolean isRefresh = (Boolean) ts.getTaskParam()
						.get("isRefresh");
				ArrayList<Map<String, Object>> favouriteList = FindUtil
						.getPopFavouriteForList(this, p, isRefresh);
	...
	private final Handler hand = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case TaskType.TS_EXAM_SEARCH_NEWS_MORE: // 查询更多新闻
				...
				break;
			case TaskType.TS_EXAM_SEARCH_NEWS_DETAIL_LOAD: // 查询新闻详细数据
				if (AppManager.getAppManager().getActivityByName(
						"NewsDetailActivity") != null) {
					AppManager.getAppManager()
							.getActivityByName("NewsDetailActivity")
							.refresh(msg.what, msg.obj, msg.arg1);
				}
				break;
			case TaskType.TS_EXAM_NEWS_FAVOURITE: // 收藏新闻
			...
			case TaskType.TS_EXAM_SEND_MOOD: // 发送心情
				if (AppManager.getAppManager().getActivityByName(
						"AddMoodActivity") != null) {
					AppManager.getAppManager()
							.getActivityByName("AddMoodActivity")
							.refresh(msg.what, msg.obj, msg.arg1);
				}
				break;
			case TaskType.TS_EXAM_PRAISE_MOOD: // 顶一下心情
