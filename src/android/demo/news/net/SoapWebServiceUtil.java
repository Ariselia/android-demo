...
public class SoapWebServiceUtil {
	static private Log log = LogFactory.getLog(SoapWebServiceUtil.class
			.getName());
	...
	public SoapWebServiceUtil(String service_name, String method_name,
			HashMap propertys) {
	...
	public SoapObject getRespondData() throws AppException {
		...
		try {
			...
			HttpTransportSE ht = new HttpTransportSE(
					CommonSetting.WebServiceUrl + SERVICE_NAME);
			ht.debug = true;
			...
			if (PROPERTYS != null) {
				...
				for (int i = 0; i < keySets.length; i++) {
					request.addProperty(keySets[i].toString(),
							PROPERTYS.get(keySets[i].toString()));
				}
			}
